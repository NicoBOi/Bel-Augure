# Bel Augure — fiche de référence

**Source unique et à jour.** Remplace toute description d'entreprise antérieure
(notamment le fichier « Description de l'entreprise » qui décrivait une SARL en
création, deux offres seulement et d'anciens prix). Tout prestataire, rédacteur
ou assistant doit se référer à ce document, et non à une version plus ancienne.

*Dernière mise à jour : 7 août 2026.*

## Identité

| | |
| --- | --- |
| Dénomination | Bel Augure |
| Forme | SARL au capital de 1 000 € |
| Immatriculation | RCS Bordeaux — SIREN 108 264 524 |
| TVA intracommunautaire | FR45 108 264 524 |
| Siège social | 83 rue Marcelin Jourdan, 33200 Bordeaux |
| Gérants | Nicolas Sempere et Corentin Crestia |
| Contact | nicolas@belaugure.studio · 06 68 49 95 04 |
| Site | https://www.belaugure.studio |

## Activité

Studio de production de films de marque pour l'hôtellerie, les spas, les thermes
et les marques de bien-être haut de gamme, à Bordeaux et partout en France.

Écriture, réalisation et postproduction sont réalisées en interne. Les deux
fondateurs sont présents à chaque étape importante du projet ; l'équipe est
complétée selon les besoins (techniciens, comédiens, équipe coulisses).

## Rôles

**Nicolas Sempere — réalisateur, directeur de la photographie, coloriste.**
Plus de cinq ans entre production, entreprise et freelance (Betclic, Better
Feeling, Sephora, Showroomprivé). Réalisation, image, lumière et étalonnage.

**Corentin Crestia — auteur, monteur, motion designer.**
Trois ans de conception de contenus pour l'entreprise et l'événementiel.
Écriture, montage, sound design et finitions en motion design.

> Ne pas décrire les fondateurs comme « deux réalisateurs » : un seul l'est.

## Offres et prix (HT)

| Offre | Pour | Prix |
| --- | --- | --- |
| **Film Signature** | Présenter la marque — film de référence de 60 à 90 s, plus versions courtes 30 s et 15 s | à partir de **5 500 €** |
| **Histoires de marque — Collection** | Une journée de tournage : au moins trois films principaux et leurs déclinaisons courtes | à partir de **3 500 €** |
| **Histoires de marque — Quatre saisons** | Une journée de tournage par mois, calendrier annuel des sujets | à partir de **3 000 € / mois**, engagement 12 mois |
| **Campagne** | Ouverture, lancement ou nouvelle identité — un film principal et au moins trois films courts | à partir de **15 000 €** |

Toute modification de prix doit être répercutée au même moment dans
`src/content/offres.js` (source unique lue par la page et par le prérendu SEO)
et dans les données structurées de `index.html`.

## Droits de diffusion

**Tous les droits sont cédés à durée illimitée dès la livraison du film.**

Formulation identique sur le site, dans les devis et dans les contrats.
Ne plus employer « offerts » ni « à vie ».

## Processus

1. Un échange de trente minutes — projet, échéance, budget.
2. Une proposition détaillée — livrables, calendrier, prix écrits avant de commencer.
3. Le tournage — équipe légère de deux à trois personnes, y compris en établissement en activité.
4. La livraison — postproduction au studio, deux séries de retours incluses.

## Ce qui n'est pas vrai à ce jour

À maintenir tel quel tant qu'aucune preuve n'existe :

- aucun cas client dans l'hôtellerie, le spa, les thermes ou le soin ;
- un seul film publié, « Les Pieds dans l'eau », auto-produit, sans commande ;
- aucun témoignage client ni résultat commercial mesuré.

Ne pas inventer de références, de chiffres de performance ou de clients.
